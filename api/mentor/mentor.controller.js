...
module.exports = {
  ...
  register: (req, res) => {
    getMentorbyEmail(req.body.email, (error, result) => {
      ...
      postMentor(req.body, (errors, results) => {
        ...
        results[0]["token"] = sign({ mentor: results }, process.env.KEYAPP, {
          algorithm: "HS256",
          expiresIn: "24h",
        });
  ...
  login: (req, res) => {
    getMentorbyEmail(req.body.email, (error, result) => {
      ...
      if (result.length == 0)
        return ERROR(res, 404, "account not found or incorrect email");
      const verif = compareSync(req.body.password, result[0].password);
      if (!verif) return ERROR(res, 403, "incorrect password");

      delete result[0].password;
      result[0]["token"] = sign({ mentor: result }, process.env.KEYAPP, {
        algorithm: "HS256",
        expiresIn: "24h",
      });
      return SUCCESS(res, 200, result);
